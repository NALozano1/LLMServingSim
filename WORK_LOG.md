...
| Date | Note |
|------|------|
| 2026-06-29 | **Hold-poller ported into profiler runner** — `run_arc_v100_profile.sh` now launches a background `gpu_freq_lock.py hold` poller on the host (outside apptainer) when `GPU_FREQ_MHZ` is set, mirroring the proven bench pattern. Replaces the single one-shot `gpu_freq_lock_apply` that was silently losing the V100 lock under load (root cause of tainted moe.csv tables). Poller covers GPUs `0..max_tp-1` (max across `TP_DEGREES` list), starts before the container, stops via `.stop` sentinel after container exits, writes `gpu_freq_hold_summary.json`. EXIT trap now calls `_cleanup()` which stops poller + force-restores clocks. No-op for bare `V100` (uncapped) runs. Verification job submitted after commit. |
...
| 2026-06-25 | Prefill-only sim-accuracy validation matrix: `run_arc_v100_prefill_validation.sh` + `submit_prefill_validation_matrix.sh`. Qwen3-30B tp4 at {uncapped, 700, 900, 1100, 1300, 1400} MHz on g049. No layer-pause; clock-locked + audited; `gpu_power` on. Tier-1 (no-DVFS uncapped) + Tier-2 (per-clock accuracy) for LLMServingSim validation. `collect_prefill_validation_results.py` aggregates results. |
| 2026-06-25 | Fixed `--sps` omission in prefill validation dataset generator call (was always required); resubmitted all validation jobs (8030500–8030511). |
| 2026-06-25 | Fixed `bench_prefill_only_enabled` scope bug in `bench/core/runner.py`: import was placed in `_drive`'s local scope but the call site is inside `_one()`, a closure in `_submit_all()` — a separate top-level function. Moved import to top of `_submit_all`. Commit `f876f194`. |
...
| 2026-06-25 | **78-run layer DVFS campaign (v100_bench_layer_dvfs_20260622) — SUPERSEDED.** 74/78 runs empty: jobs dispatched before we knew g049 is the only clock-locking node — all other V100 nodes returned `freq_apply_ok=false`. Only `scat_p01_qwen_i{0..2}` (3 runs) have data. Campaign replaced by `v100_prefill_valid_20260625` (cleaner, g049-pinned). Do not rerun. |
