#!/usr/bin/env bash
# Smoke: layer pause through prefill + decode (max 1 pause per decode forward).
#
# Uses sync vLLM (VLLM_USE_V1=0) + worker_extension_cls + external host poller.
#
#   export HF_TOKEN=...
#   bash bench/jobs/run_arc_v100_bench_layer_pause_decode_smoke.sh
#
set -euo pipefail
...
export VLLM_BENCH_PREFILL_ONLY=0
...
export DVFS_DECODE_MAX_PAUSES_PER_PASS="${DVFS_DECODE_MAX_PAUSES_PER_PASS:-1}"
export DVFS_DECODE_TOKEN_THRESHOLD="${DVFS_DECODE_TOKEN_THRESHOLD:-4}"
export DVFS_PREFILL_MIN_TOKENS="${DVFS_PREFILL_MIN_TOKENS:-8}"
...
FIX_OUTPUT_LENGTH="${FIX_OUTPUT_LENGTH:-16}"

SAFE_MODEL="$(echo "${MODEL}" | tr '/:' '__')"
OUT_DIR="${OUT_DIR:-${REPO_ROOT}/bench/results/V100_layer_pause_decode_smoke/${SAFE_MODEL}/tp${TP_SIZE}/${JOB_TAG}}"
...
echo "=== vLLM bench layer pause decode smoke ==="
echo "MODEL=${MODEL}  OUT_DIR=${OUT_DIR}"
echo "prefill+decode  FIX_OUTPUT_LENGTH=${FIX_OUTPUT_LENGTH}"
echo "DVFS_DECODE_MAX_PAUSES_PER_PASS=${DVFS_DECODE_MAX_PAUSES_PER_PASS}"
...
"$CONTAINER_RUNTIME" exec --cleanenv --nv \
  ...
  --env "DVFS_DECODE_MAX_PAUSES_PER_PASS=${DVFS_DECODE_MAX_PAUSES_PER_PASS}" \
  --env "DVFS_DECODE_TOKEN_THRESHOLD=${DVFS_DECODE_TOKEN_THRESHOLD}" \
  --env "DVFS_PREFILL_MIN_TOKENS=${DVFS_PREFILL_MIN_TOKENS}" \
...
echo "=== Verifying prefill+decode layer pause markers ==="
python3 - <<PY
import json
import sys
...
decode_steps = int("${FIX_OUTPUT_LENGTH}")
min_markers = num_layers + decode_steps
...
if metrics.get("prefill_only"):
    print("FAIL: expected prefill_only=false for decode smoke", file=sys.stderr)
    sys.exit(1)
...
print(
    f"PASS: prefill+decode layer pause smoke ({summary['marker_count']} markers, "
    f"mean pause {summary['pause_sec_mean']:.3f}s, "
    f"decode_max_pauses_per_pass=${DVFS_DECODE_MAX_PAUSES_PER_PASS})"
)
PY
