...
- **Bench campaign** `bench/campaigns/v100_bench_layer_dvfs_20260622/` — 78 runs submitted, only 4 completed (`scat_p01_qwen_i0/i1/i2`, `scat_p01_qwen_i0`). Jobs 8010073–8010150 all gone from queue. Need to check failure logs and diagnose.
...
- Jobs 8010183–8010236 — `profiler/v0/jobs/rendered/moe_*.sbatch` for H100 TP8 across Mistral, DeepSeek, Meta, Qwen MoE models. Cancelled to free queue. Re-submit when ready.
...
- **§0 re-profile** (force, fixed clock-verify framing): jobs `8021127`–`8021139`, chained.
  13 model×freq: Qwen1.5-MoE @ {700,900,1100,1300}, Qwen3-30B @ {700,900,1100,1300,1400}. tp1, `GPU_FREQ_VERIFY_STRICT=1`.
...
- **§1 per-frequency homogeneous bench**: jobs `8021235`–`8021242`, chained sequentially (no two locked-clock benches share a GPU).
  {700,900,1100,1300} × Qwen1.5-MoE. Unchanged `python -m bench run` (no pause hooks), fixed 64-tok prefill / 64-tok decode, 100 reqs.
